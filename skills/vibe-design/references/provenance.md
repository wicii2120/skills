...
This skill is self-contained at runtime. URLs document provenance and standards; operational use does not require network access. The original source audit was retrieved **2026-08-26**; Visual Style Lexicon research and Visual Style Examples were retrieved **2026-08-28**; Browser CSS Practices research was retrieved **2026-09-02**.

## linux.do terminology series

Typography, Color System, Grid & Layout, and Iconography terminology synthesize the first posts in the linux.do series by `Henry_He`. Titles are accurate English translations of authoritative JSON metadata; all publication dates match the supplied listing.

| English title | JSON timestamp | Source | Material follow-up |
| --- | --- | --- | --- |
| Design Terms for Vibe Coding — Typography | 2026-06-18T13:21:13.105Z | [Topic 2431084 JSON](https://linux.do/t/topic/2431084.json) | None |
| Design Terms for Vibe Coding — Color System | 2026-06-23T01:52:30.360Z | [Topic 2454227 JSON](https://linux.do/t/topic/2454227.json) | None |
| Design Terms for Vibe Coding — Grid & Layout | 2026-06-26T07:38:45.009Z | [Topic 2480260 JSON](https://linux.do/t/topic/2480260.json) | Author post 14: prefer logical style relationships to hard-coded values and keep style/token structure maintainable |
| Design Terms for Vibe Coding — Icons and Symbols | 2026-07-01T08:02:54.974Z | [Topic 2508074 JSON](https://linux.do/t/topic/2508074.json) | None |

The series informed terminology and communication structure, not universal product rules. No substantial source prose or non-English metadata is reproduced.
...
| Source | Pinned revision and retrieval | License | Runtime synthesis | Limits |
| --- | --- | --- | --- | --- |
| [anthropics/skills](https://github.com/anthropics/skills) | `3b3fad96af16a10759d930941b4520ba0c40edae`; 417 tracked files | No root license; relevant design skills are per-skill Apache-2.0; `frontend-design` is Apache-2.0; four document skills are source-available and excluded | `frontend-design` grounds contextual craft; canvas/theme/brand/algorithmic methods contribute philosophy, cohesive theming, parameterized exploration, and second-pass refinement; webapp-testing informs inspection | 56 binaries unverified; no source repository, font, archive, PDF, code, or binary is vendored |
| [leonxlnx/taste-skill](https://github.com/leonxlnx/taste-skill) | `ccbc15639c97057cbfcf32ecebc38ef716e4bb37`; 64 tracked files | MIT, Copyright 2026 Leonxlnx | Design Read, contextual intent dimensions, anti-default smell catalog, bounded visual families, image/art-direction options, and output-completeness behavior | 16 raster assets semantically unverified; hard bans become contextual heuristics; unsupported research/model claims are excluded |
| [emilkowalski/skills](https://github.com/emilkowalski/skills) | `d23d7f88a2e21c9e4b1418c7abe420f5c1052ba7`; 23 tracked files | MIT, Copyright 2026 Emil Kowalski | Restraint-first motion doctrine, terminology, build/review/opportunity sequence, Apple-inspired interaction principles, prototypes, and triggered web/native recipes | All files are text; exact values and package APIs remain contextual/time-sensitive; Sonner API and generic Swift guidance are excluded |

Detailed one-to-one disposition is maintenance-only in [Upstream Coverage](upstream-coverage.md).

### License and NOTICE handling

- Anthropic has no repository-wide license. Relevant incorporated design instructions were read under their pinned per-skill Apache-2.0 licenses. The relevant skills include no separate `NOTICE` file; `THIRD_PARTY_NOTICES.md` applies to upstream bundled dependencies/assets that are not redistributed here. Source-available document skills are excluded.
- MIT attribution is recorded above for Leonxlnx and Emil Kowalski. No substantial upstream prose/code, source copy, sponsor mark, or asset is redistributed.
- The runtime is a new English synthesis. Exact brand palettes, themes, code recipes, picker chrome, and binary assets were not copied.

## Browser CSS practice research

All sources in this section were retrieved **2026-09-02**. CSSWG specifications define behavior; MDN documentation and Browser Compatibility Data supply syntax and a dated interoperability signal. MDN Baseline never substitutes for the project’s browser matrix, build transforms, target-browser tests, or feature-level checks.

| Source identity | Retrieved | Authority and use | Limits |
| --- | --- | --- | --- |
| [CSS Cascade 5](https://www.w3.org/TR/css-cascade-5/), [Selectors 4](https://www.w3.org/TR/selectors-4/), and MDN [`@layer`](https://developer.mozilla.org/en-US/docs/Web/CSS/@layer) | 2026-09-02 | CSSWG cascade/layer and selector behavior plus MDN interoperability; supports explicit layer order and low-specificity defaults | Cascade 5 is a Candidate Recommendation and Selectors 4 a Working Draft; unsupported layer blocks require build/legacy handling rather than assumed source-order fallback |
| [CSS Logical Properties 1](https://www.w3.org/TR/css-logical-1/), [CSS Flexbox 1](https://www.w3.org/TR/css-flexbox-1/), and MDN [Logical properties and values](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_logical_properties_and_values) | 2026-09-02 | CSSWG flow-relative mapping and automatic minimum-size behavior plus MDN implementation reference; supports writing-direction and min-content resilience rules | Physical directions remain valid for genuinely physical relationships; overflow behavior still requires content/zoom tests |
| [CSS Containment 3](https://www.w3.org/TR/css-contain-3/), MDN [`container-type`](https://developer.mozilla.org/en-US/docs/Web/CSS/container-type), and MDN [Container queries](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_containment/Container_queries) | 2026-09-02 | CSSWG container-query behavior plus MDN syntax/support; supports size-query ownership and containment caveats | Containment 3 is a Working Draft and the MDN feature family includes uneven newer style, scroll-state, and anchored forms; runtime guidance selects only matrix-verified forms |
| [CSS Grid 2](https://www.w3.org/TR/css-grid-2/) and MDN [Subgrid](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_grid_layout/Subgrid) | 2026-09-02 | CSSWG nested-track behavior plus MDN interoperability; supports parent-track alignment when composition requires it | Grid 2 is a Candidate Recommendation Draft; subgrid still requires parent-grid and content-boundary testing |
| [Selectors 4](https://www.w3.org/TR/selectors-4/), MDN [`:has()`](https://developer.mozilla.org/en-US/docs/Web/CSS/:has), and MDN [`:focus-visible`](https://developer.mozilla.org/en-US/docs/Web/CSS/:focus-visible) | 2026-09-02 | CSSWG selector semantics plus MDN support/performance guidance; supports semantic-state styling and bounded relational selectors | Selector support does not create semantics; `:has()` specificity and invalidation cost depend on the actual selector/tree |
| [CSS Custom Properties 1](https://www.w3.org/TR/css-variables-1/), [CSS Properties and Values API 1](https://www.w3.org/TR/css-properties-values-api-1/), and MDN [`@property`](https://developer.mozilla.org/en-US/docs/Web/CSS/@property) | 2026-09-02 | CSSWG variable substitution/registration behavior plus MDN syntax/support; supports bounded token APIs and optional typed registration | Registration remained newer for long-tail matrices; it changes inheritance, invalid-value, and interpolation behavior and is not a blanket token default |
| [CSS Color 4](https://www.w3.org/TR/css-color-4/), [CSS Color Adjustment 1](https://www.w3.org/TR/css-color-adjust-1/), [Media Queries 5](https://www.w3.org/TR/mediaqueries-5/), MDN [`oklch()`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/oklch), MDN [`color-mix()`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/color-mix), and MDN [`forced-colors`](https://developer.mozilla.org/en-US/docs/Web/CSS/@media/forced-colors) | 2026-09-02 | CSSWG color, adjustment, and preference behavior plus MDN syntax/support; supports gated modern color, rendered contrast checks, and narrow forced-color repairs | CSS Color 4 is a Candidate Recommendation Draft; function parts, gamut mapping, compositing, displays, and individual media features require separate checks |
| [CSS Containment 2](https://www.w3.org/TR/css-contain-2/), MDN [`content-visibility`](https://developer.mozilla.org/en-US/docs/Web/CSS/content-visibility), and web.dev [Content visibility](https://web.dev/articles/content-visibility) | 2026-09-02 | CSSWG behavior, MDN current support, and first-party performance rationale; supports a measured optional containment branch | The web.dev article is older and its examples are not universal budgets or current support evidence; find, fragment, focus, selection, accessibility-tree, and layout side effects need target-browser verification |

WCAG 2.2 remains the normative accessibility authority. The browser sources select mechanisms and describe platform facts; they do not relax success criteria or turn dated Baseline labels into product requirements.

Selection excluded CSS nesting as authoring syntax rather than a design decision. `@scope`, style/scroll-state/anchored container queries, anchor positioning, scroll-driven animation, view transitions, masonry, and top-layer exit-animation details remain product-triggered optional branches: use them only with a material requirement, current browser-matrix evidence, progressive enhancement, and a verified fallback.

## Normative primary standards

Normative claims derive from primary standards, which take precedence over editorial sources and examples:

- [WCAG 2.2](https://www.w3.org/TR/WCAG22/) — semantics, contrast, color, reflow, text spacing, targets, input, motion, flashing, status, errors, and non-text content.
- [WAI-ARIA 1.2](https://www.w3.org/TR/wai-aria-1.2/) and [Accessible Name and Description Computation 1.2](https://www.w3.org/TR/accname-1.2/) — roles, states, names, and descriptions where native semantics are insufficient.
- [HTML Living Standard](https://html.spec.whatwg.org/) — semantic elements, controls, embedded content, and responsive images.
- [CSS Cascade 5](https://www.w3.org/TR/css-cascade-5/), [Selectors 4](https://www.w3.org/TR/selectors-4/), [CSS Logical Properties 1](https://www.w3.org/TR/css-logical-1/), [CSS Custom Properties 1](https://www.w3.org/TR/css-variables-1/), and [CSS Properties and Values API 1](https://www.w3.org/TR/css-properties-values-api-1/) — cascade, selector, flow-relative, variable-substitution, and registered-property behavior.
- [CSS Color 4](https://www.w3.org/TR/css-color-4/), [CSS Color Adjustment 1](https://www.w3.org/TR/css-color-adjust-1/), [CSS Values and Units 4](https://www.w3.org/TR/css-values-4/), [CSS Fonts 4](https://www.w3.org/TR/css-fonts-4/), [CSS Text 3](https://www.w3.org/TR/css-text-3/), [CSS Grid 1](https://www.w3.org/TR/css-grid-1/), [CSS Grid 2](https://www.w3.org/TR/css-grid-2/), [CSS Flexbox 1](https://www.w3.org/TR/css-flexbox-1/), [CSS Containment 2](https://www.w3.org/TR/css-contain-2/), [CSS Containment 3](https://www.w3.org/TR/css-contain-3/), and [Media Queries 5](https://www.w3.org/TR/mediaqueries-5/) — portable web rendering, layout, containment, color, type, and user preferences.
- [SVG 2](https://www.w3.org/TR/SVG2/) — vector icon/graphic semantics and rendering.
- [Design Tokens Format Module](https://tr.designtokens.org/format/) from the W3C Design Tokens Community Group — token types, groups, values, and aliases.

Cite exact WCAG success criteria for normative requirements. Stricter values are product requirements when explicitly adopted.
...
The source-authority order in `SKILL.md` governs conflicts. Raw retrievals and source repositories remain outside this repository.
